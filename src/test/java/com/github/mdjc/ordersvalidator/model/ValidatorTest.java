...
public class ValidatorTest extends TestCase {
	...
	public void setUp() {
		...
		validator = new Validator(validSymbols, BROKER_ORDERS_PER_MINUTE);
	}

	public void testIsValidGivenOrderWithNullOrEmptyStringReturnsFalse() {
		...
		assertFalse(validator.isValid(new Order(timeStamp, "", sequence, type, symbol, quantity, price, side)));
		...
		assertFalse(validator.isValid(new Order(timeStamp, broker, sequence, "", symbol, quantity, price, side)));
		assertFalse(validator.isValid(new Order(timeStamp, broker, sequence, type, null, quantity, price, side)));
		assertFalse(validator.isValid(new Order(timeStamp, broker, sequence, type, "", quantity, price, side)));
		...
		assertFalse(validator.isValid(new Order(timeStamp, broker, sequence, type, symbol, quantity, price, "")));
	...
	public void testIsValidNonExcedingOrdersPerMinuteLimitReturnsTrue() {
		IntStream.rangeClosed(1, BROKER_ORDERS_PER_MINUTE)
